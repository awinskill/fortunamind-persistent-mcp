#!/usr/bin/env python3
"""
FortunaMind Persistent MCP HTTP Bridge

Proxies stdio MCP protocol to HTTP MCP server with both subscription 
and Coinbase API credential injection.
...
class PersistentMCPHTTPBridge:
    ...
    async def run(self):
        ...
        logger.info(f"🎫 Subscription key: {self.subscription_key[:20]}..." if self.subscription_key else "🎫 Subscription key: None")
        logger.info(f"🔑 Coinbase API key: {self.coinbase_api_key[:30]}..." if self.coinbase_api_key else "🔑 Coinbase API key: None")
        logger.info(f"🔐 Coinbase API secret: {'*' * (len(self.coinbase_api_secret) if self.coinbase_api_secret else 0)}")
